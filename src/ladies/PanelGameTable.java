...
public class PanelGameTable extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (estado == turno) {
			if (!(moveDone && botonSelectedId != origen)) {
				button.setBackground(Color.BLUE);
				if (origen != -1)
					arrayButtons[origen].setBackground(Color.BLACK);
				/*
				 * //Comprovoar si estas obligado a matar
				 * if(obligadoAMatar&&laFichaSeleccionadaNoEsLaCorrecta){origen=
				 * 0;} else{ cambiar imagen boton a seleccionado deseleccionar
				 * otros }
				 */
				...
				if (botonSelectedId == origen && moveDone == true) {
					...
					if (turno == BLANCAS) {
						turno = NEGRAS;
						//JOptionPane.showMessageDialog(null, "Turno de la Negras");
					} else {
						turno = BLANCAS;
						//JOptionPane.showMessageDialog(null, "Turno de la Blancas");
					}
					origen = -1;
					moveDone = false;
				} else {
		...
		} else if (estado == PanelGameTable.EMPTY && origen != -1) {
